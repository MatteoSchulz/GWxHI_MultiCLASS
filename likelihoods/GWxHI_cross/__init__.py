...
class GWxHI_cross(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        self.cl_file = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}{binned_str}_Cl.pkl'
        self.Cov_file = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}{binned_str}_Cov.pkl'
        self.InvCov_file = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}{binned_str}_InvCov.pkl'
        
        self.redshift_file = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}_{self.network}_zbin.pkl'
        self.redshift_filehi = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}_{self.network}_zbinhi.pkl'
        self.redshift_filegw = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}_{self.network}_zbingw.pkl'
        
        self.zedges_filehi = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}_{self.network}_zedgeshi.pkl'
        self.zedges_filegw = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}_{self.network}_zedgesgw.pkl'
        
        self.Ngw_file = f'dataset_GWxHI_cross_{self.bias}_{self.method}_{self.nbins}_{self.network}_NgwBin.pkl'
    ...
    def loglkl(self, cosmo, data):
        ...
        try:
            ...
            # ==========================================
            idx_hh_end = int(N_iter_hh)
            idx_hg_end = int(N_iter_hh + N_iter_hg)
            
            # Initialize trackers for the isolated chi2 sums
            chi2_hg_isolated = 0.
            ...
            for i in range(len(chi2)):
                ...
                cov_i = np.array([[cell[i] for cell in row] for row in self.Cov], dtype=float)

                delta_hg = delta_cl_i[idx_hh_end : idx_hg_end]
                cov_hg = cov_i[idx_hh_end:idx_hg_end, idx_hh_end:idx_hg_end]

                chi2_hg_isolated += np.dot(delta_hg, np.dot(np.linalg.inv(cov_hg), delta_hg))
                
            #chi2_total = np.sum(chi2)
            #log_likelihood = -0.5 * chi2_total
            
            # Separated hg likelihood
            log_likelihood_hg = -0.5 * chi2_hg_isolated

            return log_likelihood_hg
